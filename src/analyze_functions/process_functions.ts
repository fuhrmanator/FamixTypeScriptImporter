...
function processFile(f: SourceFile): void {
    const isModule = Helpers.isModule(f);
...
/**
     ...
     */
export function processImportClauses(modules: Array<SourceFile>, exports: Array<ReadonlyMap<string, ExportedDeclarations[]>>): void {
    logger.info(`processImportClauses: Creating import clauses:`);
    modules.forEach(module => {
        module.getImportDeclarations().forEach(impDecl => {
            const path = Helpers.getModulePath(impDecl);
