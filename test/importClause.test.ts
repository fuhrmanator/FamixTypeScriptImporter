...
describe('Tests for import clauses', () => {
    ...
    it("should import myRenamedDefaultClassW that is a renamed ClassW from module complexExportModule.ts", () => {
        ...
        const importClause = importClauses.find(e => e.importedEntity?.getName() === 'myRenamedDefaultClassW');
    ...
    it("should import ClassX from module that exports from an import", () => {
        ...
        const importClause = importClauses.find(e => e.importedEntity?.getName() === 'ClassX');
        expect(importClause).toBeTruthy();
        // importing entity is reImporterModule.ts
        expect(importClause?.importingEntity).toBeTruthy();
        expect(importClause?.importingEntity?.getName()).toBe("reImporterModule.ts");
    });

    it("should import ClassZ from module complexExporterModule.ts", () => {
        // find the import clause for ClassZ
        const importClause = importClauses.find(e => e.importedEntity?.getName() === 'ClassZ');
        expect(importClause).toBeTruthy();
        // importing entity is multipleClassImporterModule.ts
        expect(importClause?.importingEntity).toBeTruthy();
        expect(importClause?.importingEntity?.getName()).toBe("multipleClassImporterModule.ts");
    });

    it("should have a default import clause for test", () => {
        expect(importClauses).toBeTruthy();
        // find the import clause for ClassW
        const importClause = importClauses.find(e => e.importedEntity?.getName() === 'test');
        expect(importClause).toBeTruthy();
        // importing entity is complexExportModule.ts
        expect(importClause?.importingEntity).toBeTruthy();
        expect(importClause?.importingEntity?.getName()).toBe("defaultImporterModule.ts");
    });
    ...
    it("should contain one imports element for module oneClassExporter.ts", () => {
        ...
        expect(exportedEntity?.getIncomingImports()?.values().next().value.importingEntity?.getName()).toBe("oneClassImporter.ts");
    });
    ...
    it("should have an import clause for require('foo')", () => {
        ...
        const importClause = importClauses.find(e => e.importedEntity?.getName() === 'foo');
        expect(importClause).toBeTruthy();
        // importing entity is lazyRequireModuleCommonJS.ts
        expect(importClause?.importingEntity).toBeTruthy();
        expect(importClause?.importingEntity?.getName()).toBe("lazyRequireModuleCommonJS.ts");
        // const fileAnchor = importClause?.getSourceAnchor() as IndexedFileAnchor;
        ...
        expect((importClause?.importedEntity as StructuralEntity).getDeclaredType()?.getName()).toBe("any");
    });
